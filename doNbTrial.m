...
Screen('FillOval', display.w, display.black, params.fixLoc); %fixation dot
Screen('DrawTexture', display.w, tex.texID(trial),[], params.stimLoc(trial,:)); 
Screen('Flip', display.w); 
...
SetMouse(display.centerCoords(1),display.centerCoords(2),display.w);
...
if history.respAngle(end) < 0
    history.respAngle(end) = 360+history.respAngle(end);
end
end
